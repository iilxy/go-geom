...
func (ees *edgeEndStarCommon) computeLabelling(geomGraph []GeometryGraph) {
	...
	ees.edgeMap.Walk(func(key, value interface{}) {
		...
		for geomi := 0; geomi < 2; geomi++ {
			if label[geomi].isLine() && label[geomi][OnLabel] == location.Boundary {
...
func (ees *edgeEndStarCommon) checkAreaLabelsConsistent(geomIndex int) (bool, error) {
	...
	startLoc := startLabel[geomIndex][LeftOfLabel]
	...
	ees.edgeMap.WalkInterruptible(func(key, value interface{}) bool {
		...
		leftLoc := label[geomIndex][LeftOfLabel]
		rightLoc := label[geomIndex][RightOfLabel]
...
func (ees *edgeEndStarCommon) propagateSideLabels(geomIndex int) error {
	...
	ees.edgeMap.Walk(func(key, value interface{}) {
		e := value.(edgeEnd)
		label := e.getLabel()
		if label[geomIndex].isArea() && label[geomIndex][LeftOfLabel] != location.None {
			startLoc = label[geomIndex][LeftOfLabel]
		}
	})
	...
	ees.edgeMap.Walk(func(key, value interface{}) {
		...
		if label[geomIndex][OnLabel] == location.None {
			label[geomIndex][OnLabel] = currLoc
		}
		...
		if label[geomIndex].isArea() {
			 ...
			 */
			if label[geomIndex][LeftOfLabel] != location.None {
				err = fmt.Errorf("found single null side")
			}
			label[geomIndex][RightOfLabel] = currLoc
			label[geomIndex][LeftOfLabel] = currLoc
		}
	})
