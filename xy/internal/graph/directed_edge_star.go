...
func (des *directedEdgeStar) computeLabelling(geom []GeometryGraph) {
	...
	des.edgeMap.Walk(func(key, value interface{}) {
		...
		eLabel := e.label
		for i := 0; i < 2; i++ {
			eLoc := eLabel[i][OnLabel]
			if eLoc == location.Interior || eLoc == location.Boundary {
				des.label[i][OnLabel] = location.Interior
			}
		}
	})
...
func (des *directedEdgeStar) updateLabelling(nodeLabel *Label) {
	des.edgeMap.Walk(func(key, ee interface{}) {
		de := ee.(directedEdge)
		label := de.label
		label[0].setAllLocationsIfNull(nodeLabel[0][OnLabel])
		label[0].setAllLocationsIfNull(nodeLabel[1][OnLabel])
	})
}
...
func (des *directedEdgeStar) computeDepths(de *directedEdge) error {
	...
	startDepth := de.depth[LeftOfLabel]
	targetLastDepth := de.depth[RightOfLabel]
...
func (des *directedEdgeStar) computeDepthsForEdges(startIndex, endIndex, startDepth int) int {
	...
	des.edgeMap.Walk(func(key, value interface{}) {
		...
		nextDe.setEdgeDepths(RightOfLabel, currDepth)
		currDepth = nextDe.depth[LeftOfLabel]
	})
