...
func (de *directedEdge) setDepth(pos LabelPos, depthVal int) {
	if de.depth[pos] != -999 {
...
func (de *directedEdge) isInteriorAreaEdge() bool {
	...
	for i := 0; i < 2; i++ {
		if !(de.label[i].isArea() && de.label[i][LeftOfLabel] == location.Interior && de.label[i][RightOfLabel] == location.Interior) {
...
func (de *directedEdge) setEdgeDepths(pos LabelPos, depth int) {
...
func (de *directedEdge) String() string {
	s := fmt.Sprintf("%v %v/%v (%v) ", de.edgeEndCommon.String(), de.depth[LeftOfLabel], de.depth[RightOfLabel], de.getDepthDelta())
